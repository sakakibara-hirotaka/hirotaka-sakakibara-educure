...
public class MeetingRoomVilidator implements Validator{
    ...
    public void validate(Object target, Errors errors) {
        ...
        if(form.getStartDateTime() == null) {
            errors.rejectValue("startDateTime", "required", "利用開始日時は必須です");
            return;
        }

        if(form.getEndDateTime() == null) {
            errors.rejectValue("endDateTime", "required", "利用終了日時は必須です");
            return;
        }

        if(form.getPeopleCount() == null) {
            errors.rejectValue("peopleCount", "required", "利用人数は必須です");
            return;
        }
        ...
        Duration duration = Duration.between(form.getStartDateTime(), form.getEndDateTime());
